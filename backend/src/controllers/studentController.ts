...
import { Resume, StudentProfile } from '../models/index.js';
...
export async function uploadResume(request: Request, response: Response) {
  ...
  if (file.mimetype !== 'application/pdf' || !file.originalname.toLowerCase().endsWith('.pdf') || file.size > 5 * 1024 * 1024 || file.buffer.subarray(0, 4).toString() !== '%PDF') {
...
export async function getResumes(request: Request, response: Response) { const profile = await getOrCreateStudentProfile(studentId(request)); return success(response, await Resume.find({ student: profile.profile._id }).select('-storageKey').sort({ createdAt: -1 }).lean()); }
export async function setPrimaryResume(request: Request, response: Response) { const profile = await getOrCreateStudentProfile(studentId(request)); const resume = await Resume.findOne({ _id: request.params.id, student: profile.profile._id, status: 'ACTIVE' }); if (!resume) throw new AppError(404, 'Resume not found', 'RESUME_NOT_FOUND'); await Resume.updateMany({ student: profile.profile._id }, { $set: { isPrimary: false } }); resume.isPrimary = true; await resume.save(); return success(response, resume); }
export async function archiveResume(request: Request, response: Response) { const profile = await getOrCreateStudentProfile(studentId(request)); const resume = await Resume.findOneAndUpdate({ _id: request.params.id, student: profile.profile._id }, { $set: { status: 'ARCHIVED', isPrimary: false } }, { new: true }).select('-storageKey'); if (!resume) throw new AppError(404, 'Resume not found', 'RESUME_NOT_FOUND'); return success(response, resume); }
