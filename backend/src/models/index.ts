...
const applicationSchema = new Schema({ student: { type: objectId, ref: 'StudentProfile', required: true, index: true }, drive: { type: objectId, ref: 'PlacementDrive', required: true, index: true }, currentStage: { type: String, enum: ['APPLIED', 'UNDER_REVIEW', 'SHORTLISTED', 'APTITUDE', 'CODING', 'TECHNICAL', 'GD', 'HR', 'SELECTED', 'REJECTED', 'WITHDRAWN', 'PLACED'], default: 'APPLIED', index: true }, appliedAt: { type: Date, default: Date.now }, withdrawnAt: Date }, { timestamps });
...
const interviewSchema = new Schema({ drive: { type: objectId, ref: 'PlacementDrive', required: true, index: true }, application: { type: objectId, ref: 'Application', index: true }, type: { type: String, required: true }, date: { type: Date, required: true, index: true }, mode: { type: String, enum: ['ONSITE', 'ONLINE'], required: true }, venueOrLink: String, instructions: String }, { timestamps });
const attendanceSchema = new Schema({ interview: { type: objectId, ref: 'Interview', required: true, index: true }, student: { type: objectId, ref: 'StudentProfile', required: true, index: true }, status: { type: String, enum: ['PRESENT', 'ABSENT', 'EXCUSED'], required: true }, markedBy: { type: objectId, ref: 'User', required: true }, markedAt: { type: Date, default: Date.now } }, { timestamps });
const documentSchema = new Schema({ owner: { type: objectId, ref: 'User', required: true, index: true }, type: { type: String, required: true }, storageKey: { type: String, required: true }, originalName: String, mimeType: String, sizeBytes: Number, status: { type: String, enum: ['PENDING', 'VERIFIED', 'REJECTED', 'REUPLOAD_REQUIRED'], default: 'PENDING', index: true }, reviewedBy: { type: objectId, ref: 'User' }, reviewedAt: Date, reviewComment: String }, { timestamps });
...
const placementResultSchema = new Schema({ application: { type: objectId, ref: 'Application', required: true, unique: true }, status: { type: String, enum: ['SELECTED', 'REJECTED', 'OFFER', 'PLACED'], required: true }, packageCtc: Number, joiningDate: Date, notes: String, recordedBy: { type: objectId, ref: 'User', required: true } }, { timestamps });
...
const preparationResourceSchema = new Schema({ title: { type: String, required: true }, category: { type: String, required: true, index: true }, url: String, description: String, isPublished: { type: Boolean, default: false, index: true } }, { timestamps });
